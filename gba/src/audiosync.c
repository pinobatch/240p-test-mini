...
void activity_audio_sync() {
  ...
  do {
    ...
    SOAM[0].attr0 = OBJ_Y(y) | ATTR0_COLOR_16 | ATTR0_SQUARE;
    ...
    for (unsigned int i = 0; i < 6; ++i) {
      BG_PALETTE[i] = progress >= min_progress[i] ? RGB5(31, 31, 31) : RGB5(0, 0, 0);
    }
    ...
    for (unsigned int i = 0; i < 6; ++i) {
      BG_PALETTE_SUB[i] = progress >= min_progress[i] ? RGB5(31, 31, 31) : RGB5(0, 0, 0);
    }
    SPRITE_PALETTE_SUB[1] = RGB5(31, 31, 31);
    #endif
    ppu_copy_oam();
