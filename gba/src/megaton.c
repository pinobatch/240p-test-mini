...
static void megaton_draw_boolean(NAMETABLE *map_address, unsigned int y, unsigned int value) {
...
static void megaton_draw_reticle(unsigned int x, unsigned int y) {
  ...
  y = OBJ_Y(y) | ATTR0_COLOR_16 | ATTR0_SQUARE;
  ...
  SOAM[i].attr0 = y;
  SOAM[i].attr1 = x + 16 + ATTR1_FLIP_X;
  SOAM[i++].attr2 = RETICLE_TILE;
  y += 16;
  SOAM[i].attr0 = y;
  SOAM[i].attr1 = x + ATTR1_FLIP_Y;
  SOAM[i++].attr2 = RETICLE_TILE;
  SOAM[i].attr0 = y;
  SOAM[i].attr1 = x + 16 + ATTR1_FLIP_X + ATTR1_FLIP_Y;
  SOAM[i++].attr2 = RETICLE_TILE;
  oam_used = i;
}

static void megaton_draw_variable_data(NAMETABLE *map_address, unsigned int y, unsigned int dir, unsigned int randomize, unsigned int with_audio) {
    // Draw the cursor
...
static void megaton_draw_static_reticle(NAMETABLE *map_address) {
...
void activity_megaton() {
  ...
  megaton_draw_static_reticle(MAP);
  // Make space for results
  dma_memset16(PATRAM4(0, 0x30), 0x0000, 32 * 2 * NUM_TRIALS);
  loadMapRowMajor(&(MAP[PFMAP][2][2]), 0x30, 2, NUM_TRIALS);
  #if defined (__NDS__) && (SAME_ON_BOTH_SCREENS)
  megaton_draw_static_reticle(MAP_SUB);
  // Make space for results
  dma_memset16(PATRAM4_SUB(0, 0x30), 0x0000, 32 * 2 * NUM_TRIALS);
  loadMapRowMajor(&(MAP_SUB[PFMAP][2][2]), 0x30, 2, NUM_TRIALS);
  #endif
  ...
  do {
    ...
    if (new_keys & KEY_A) {
      ...
      megaton_print_values(PATRAM4(0, 0x30 + progress * 2), value, early);
      #if defined (__NDS__) && (SAME_ON_BOTH_SCREENS)
      megaton_print_values(PATRAM4_SUB(0, 0x30 + progress * 2), value, early);
      #endif
    ...
    BG_PALETTE[0] = (x == 128 && with_audio) ? RGB5(31, 31, 31) : RGB5(0, 0, 0);
    BG_PALETTE[1] = SPRITE_PALETTE[1] = RGB5(31, 31, 31);
    BG_PALETTE[2] = RGB5(20, 25, 31);
    #if defined (__NDS__) && (SAME_ON_BOTH_SCREENS)
    ...
    BG_PALETTE_SUB[0] = (x == 128 && with_audio) ? RGB5(31, 31, 31) : RGB5(0, 0, 0);
    BG_PALETTE_SUB[1] = SPRITE_PALETTE_SUB[1] = RGB5(31, 31, 31);
    BG_PALETTE_SUB[2] = RGB5(20, 25, 31);
    #endif
    ppu_copy_oam();
    
    megaton_draw_variable_data(MAP, y, dir, randomize, with_audio);
    #if defined (__NDS__) && (SAME_ON_BOTH_SCREENS)
    megaton_draw_variable_data(MAP_SUB, y, dir, randomize, with_audio);
    #endif
  ...
  dma_memset16(MAP[PFMAP][(SCREEN_HEIGHT - 40)>> 3], BLANK_TILE, 32*4*2);
  #if defined (__NDS__) && (SAME_ON_BOTH_SCREENS)
  dma_memset16(MAP_SUB[PFMAP][(SCREEN_HEIGHT - 40)>> 3], BLANK_TILE, 32*4*2);
  #endif
